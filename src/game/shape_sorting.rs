...
element_for! {
  pub struct ShapeSorting for State {
    ...
    drop_target: Cell<Option<usize>> = Cell::new(None),
  }
}
        ...
        el
            ...
            .internal_on_mouse(|el, state: &mut State, event| {
                ...
                if event.mouse == Mouse::Drag {
                    ...
                    // Remember the hovered drop box: touch lift coordinates
                    // shift cells, so Up-time `entered` can't be trusted.
                    // Only overwrite while over a box, keeping last known.
                    let drop_box_count = frames.len().saturating_sub(1);
                    for (idx, frame) in frames.iter().enumerate().take(drop_box_count) {
                        if frame.status.entered.get() {
                            el.drop_target.set(Some(idx));
                            break;
                        }
                    }
                ...
                if event.mouse == Mouse::Up {
                    if let Some((_, shape)) = el.elements.sot::<ColoredShape<State>>() {
                        ...
                        // Prefer the box remembered during Drag over live `entered`.
                        if let Some(idx) = el.drop_target.get() {
                            if let Some(frame) = frames.get(idx) {
                                if Self::is_match(frame, &shape) && frame.elements.len() < 4 {
                                    frame.elements.inner.borrow_mut().push(shape.clone());
                                    frame
                                        .elements_snap_left()
                                        .elements_snap_top()
